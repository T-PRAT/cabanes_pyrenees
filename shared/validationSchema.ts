...
export const hutSchema = z.object({
   ...
   summerCapacity: z.coerce.number().int().min(0, { message: 'La capacité été doit être au moins 0' }),
   winterCapacity: z.coerce.number().int().min(0, { message: 'La capacité hiver doit être au moins 0' }),
   altitude: z.coerce.number().int().min(0, { message: "L'altitude doit être au moins 0" }).max(3404, { message: "L'altitude ne doit pas dépasser 3404 mètres" }),
